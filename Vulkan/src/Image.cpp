...
KGR::_Vulkan::Image::Image(uint32_t width, uint32_t height, vk::Format format, vk::ImageTiling tiling,
	vk::ImageUsageFlags usage, vk::MemoryPropertyFlags properties, Device* device, PhysicalDevice* physicalDevice) : m_width(width),m_height(height)
{
	createImage(width, height, format, tiling, usage, properties, m_image, m_imageMemory, device, physicalDevice);
}

void KGR::_Vulkan::Image::CreateView(vk::Format format, vk::ImageAspectFlags aspectFlags, Device* device)
{
	m_imageView = createImageView(m_image, format, aspectFlags, device);
}
...
void KGR::_Vulkan::Image::createImage(uint32_t width, uint32_t height, vk::Format format, vk::ImageTiling tiling,
